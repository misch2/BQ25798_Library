...
#include "../../src/utility/gen_others.h"
#include "../../src/utility/gen_registers.h"
...
#include "../../src/utility/gen_settings.h"  // This file is generated by the template engine (Jinja2)
  ...
  void writeSettingToReg8(int reg, uint8_t value, int bitLength, int bitShift);
  uint16_t readSettingFromReg16(int widereg, int bitLength, int bitShift);
  void writeSettingToReg16(int widereg, uint16_t value, int bitLength, int bitShift);
...
template <class C>
uint8_t BQ25798Core<C>::readSettingFromReg8(int reg, int bitLength, int bitShift) {
  uint8_t regval = this->i2c_readReg8(reg);

  DEBUG_PRINT("[readSettingFromReg8] [reg#=0x%02X, bitLength=%d, bitShift=%d], reg content = 0x%02X\n", reg, bitLength, bitShift, regval);
  DEBUG_PRINT("[readSettingFromReg8]  -> retval = %02X\n", (regval >> bitShift) & BITLENGTH_TO_MASK(bitLength));
  ...
  return (regval >> bitShift) & BITLENGTH_TO_MASK(bitLength);
}

template <class C>
void BQ25798Core<C>::writeSettingToReg8(int reg, uint8_t value, int bitLength, int bitShift) {
...
template <class C>
uint16_t BQ25798Core<C>::readSettingFromReg16(int widereg, int bitLength, int bitShift) {
  uint16_t regval = this->i2c_readReg16(widereg);
  if (this->i2c_isError()) {
...
void BQ25798Core<C>::writeSettingToReg16(int widereg, uint16_t value, int bitLength, int bitShift) {
  ...
  regval &= negatedMask;
  regval |= shiftedValue;
  this->i2c_writeReg16(widereg, regval);
  if (this->i2c_isError()) {
    DEBUG_PRINT("[writeSettingToReg16]  -> ERROR\n");
    _setError(Error::ERROR_I2C_WRITE);
    return;
...
uint16_t BQ25798Core<C>::getRaw(const Setting& setting) {
  ...
  if (setting.long_reg) {
    raw_value = readSettingFromReg16(setting.reg, setting.bitlength, setting.bitshift);
  } else {
    raw_value = readSettingFromReg8(setting.reg, setting.bitlength, setting.bitshift);
  }
...
void BQ25798Core<C>::setAndWriteRaw(const Setting& setting, uint16_t value) {
  ...
  if (setting.long_reg) {
    ...
    writeSettingToReg8(setting.reg, value, setting.bitlength, setting.bitshift);
  }
}
